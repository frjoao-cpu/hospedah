...
(function () {
    ...
    function runAttempt() {
      return fetch(url, options).then(function (res) {
        if (res.status !== 429 || attempt >= GEMINI_MAX_RETRIES) {
          if (res.status === 429) {
            console.error('[HOSPEDAH_AI] ' + requestLabel + ' rate-limit — todas as ' + totalAttempts + ' tentativas esgotadas.');
          }
          return res;
        }

        var waitMs = GEMINI_RETRY_BASE_MS * (attempt + 1);
        console.warn('[HOSPEDAH_AI] ' + requestLabel + ' rate-limit 429 (tentativa ' + (attempt + 1) + '/' + totalAttempts + ') — tentando novamente em ' + waitMs + ' ms.');
        attempt++;
  ...
  function buildPayload(lead, mensagens, intencao, temperature, faqExtras) {
    // Build rawContents: only the spoken text of each turn is sent.
    // content is coerced to string to guard against null/undefined values
    // that would produce {"text":null} and trigger a 400 from the API.
    ...
    // Gemini requires the first entry in contents to have role 'user'.
    var firstUserIdx = -1;
    ...
    // Merge consecutive same-role messages (edge case, e.g. two user messages
    // in a row before an AI reply).  Skip entries with empty text.
    var contents = [];
    for (var j = 0; j < trimmed.length; j++) {
      var c = trimmed[j];
      var cPartText = (c.parts.length > 0 && c.parts[0].text) ? c.parts[0].text : '';
      if (!cPartText) continue; // skip empty turns
      if (contents.length > 0 && contents[contents.length - 1].role === c.role) {
        contents[contents.length - 1].parts[0].text += '\n' + cPartText;
      } else {
        contents.push({ role: c.role, parts: [{ text: cPartText }] });
      }
    }
  ...
  function chamar(lead, mensagens, intencao, temperature, faqExtras) {
    ...
    return fetchGeminiWithRetry(GEMINI_BASE + ':generateContent?key=' + key, {
      method: 'POST',
