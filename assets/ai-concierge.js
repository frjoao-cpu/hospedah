...
(function () {
  ...
  function buildPayload(lead, mensagens, intencao, temperature, faqExtras) {
    // Build rawContents — model turns include thought parts when present so that the
    // Gemini 2.5 Flash API can properly reconstruct the conversation state.
    var rawContents = (mensagens || []).map(function (m) {
      if (m.role === 'assistant') {
        var parts = [];
        if (m.thoughts) parts.push({ thought: true, text: m.thoughts });
