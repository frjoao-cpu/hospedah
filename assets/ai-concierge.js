...
(function () {
  ...
  // Side-channel for the most recent model thought tokens (set after every chamar/chamarEdge call).
  // Callers read window.HOSPEDAH_AI._lastThoughts immediately after the Promise resolves.
  var _lastThoughts = null;
  ...
  function buildPayload(lead, mensagens, intencao, temperature, faqExtras) {
    // Build rawContents — only the assistant's spoken text is sent back as a
    // model turn.  Gemini's `{thought: true, text: ...}` summary parts are
    // OUTPUT-ONLY: the API rejects requests whose `contents` contain them,
    // which previously caused every turn after the 1st to fail (issues #205,
    // #208, #209).  Thoughts are still emitted (via includeThoughts) and
    // exposed to the UI through the side-channel for the current turn — they
    // are simply not echoed back into the next request.
    ...
    // Merge consecutive same-role messages (edge case).  For model turns that may
    // carry thought parts, only the non-thought text part is merged.
    var contents = [];
    for (var j = 0; j < trimmed.length; j++) {
      var c = trimmed[j];
      if (!c.parts.length) continue;
      if (contents.length > 0 && contents[contents.length - 1].role === c.role) {
        // Find the text (non-thought) part in each and merge
        var lastParts = contents[contents.length - 1].parts;
        var lastText = null;
        for (var li = 0; li < lastParts.length; li++) {
          if (!lastParts[li].thought) { lastText = lastParts[li]; break; }
        }
        var cText = null;
        for (var ci = 0; ci < c.parts.length; ci++) {
          if (!c.parts[ci].thought) { cText = c.parts[ci]; break; }
        }
        if (lastText && cText) {
          lastText.text += '\n' + cText.text;
        } else if (cText) {
        ...
        contents.push({
          ...
          parts: c.parts.map(function (p) {
            return p.thought ? { thought: true, text: p.text } : { text: p.text };
          })
        });
      }
    }
  ...
  function chamarEdge(lead, mensagens, intencao, temperature, faqExtras) {
    ...
    .then(function (res) {
      return res.json().then(function (data) {
        ...
        _lastThoughts = (data && data.thoughts) ? data.thoughts : null;
    ...
    .catch(function () { _lastThoughts = null; return null; });
  }
    ...
    .then(function (res) {
      ...
      return res.json().then(function (data) {
        ...
        for (var i = 0; i < parts.length; i++) {
          // Collect thought tokens for multi-turn history round-trip
          if (parts[i].thought && parts[i].text) {
            thoughtText += parts[i].text;
          } else if (parts[i].text && !parts[i].thought && !textPart) {
            textPart = parts[i];
          }
        }
        ...
        _lastThoughts = thoughtText || null;
  ...
  window.HOSPEDAH_AI = { chamar: chamar, buildPayload: buildPayload, streamUrl: streamUrl,
    get _lastThoughts() { return _lastThoughts; } };
})();
